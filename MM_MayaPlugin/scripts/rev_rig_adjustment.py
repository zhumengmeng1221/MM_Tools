...
def toolkit(_):
	...
	cmds.select( clear=True )
	cmds.confirmDialog( title='骨架贴合', message='骨架已移动', button=['Alright'] )
...
def copySkinWeightsUV(_):
	...
	cmds.select( clear=True )
	cmds.confirmDialog( title='复制蒙皮权重', message='复制的蒙皮权重', button=['Dismiss'] )
...
def exportWeightsdiceat(*args): 
	...
	cmds.confirmDialog( title='复制蒙皮权重', message='使用字典复制的蒙皮权重', button=['Dismiss'] )
...
def RevUI(*args):
	...
	cmds.text(label="将装配的骨架传递到缩放或修改的网格",  width=450, height=50, align="center", fn="boldLabelFont")

	#selecting the mesh UI
	cmds.columnLayout("col_selectmesh", parent="AllLayout")
	cmds.text(label="第一部分",  width=450, height=30, backgroundColor=[0.274, 0.619, 0.920], align="center")
	cmds.text(label="选择并单击相应的按钮.",  width=450, height=30, align="left")
	...
	cmds.button("selectHeadBtn", label="<<    原始模型", width = 150, height=26, command=button_wrapper(store_og, headmeshTXT))
	...
	cmds.button("selectTeethBtn", label="<<    修改后模型", width = 150, height=26, command=button_wrapper(store_new, teethmeshTXT))
	...
	cmds.button("selectJointBtn", label="<<    Root 骨骼", width = 150, height=26, command=button_wrapper(store_joint, teethmeshTXT))


	#Fit skeleton button
	cmds.columnLayout("col_getbs", parent="AllLayout")
	cmds.text(label="如果选择了“全部”，则运行下方按钮",  width=450, height=30, align="center")
	...
	cmds.button(width=225, label="匹配骨骼", backgroundColor=[0.80, 0.65, 0.0], align="center", command=  toolkit)
	cmds.text(label="        ",  width=112, height=5, align="center")  #empty space to align the button in middle


	#Part II
	cmds.columnLayout("col_skin", parent="AllLayout")
	cmds.text(label="第二部分",  width=450, height=30, backgroundColor=[0.274, 0.619, 0.920], align="center")
	cmds.text(label="一旦点击, 将会按按钮方式进行蒙皮",  width=450, height=30, align="center")

	cmds.rowLayout("row_skin", numberOfColumns=2,  width=450, height=50, parent="col_skin")
	cmds.button(label="UV 方式", backgroundColor=[0.80, 0.65, 0.0], align="left", width = 225, height=26, command=  copySkinWeightsUV)
	cmds.button(label="字典方法（慢速）", backgroundColor=[0.80, 0.65, 0.0], align="center", width = 225, height=26, command=  exportWeightsdiceat)

	cmds.columnLayout("col_note", parent="AllLayout")
	cmds.text(label="              ",  width=450, height=30, align="left")
	cmds.text(label=" 注意：旧网格和新网格应具有相同的拓扑和顶点ID",  width=450, height=30, align="left", bgc=[0.79, 0.45, 0.45])
	cmds.text(label="              ",  width=450, height=5, align="left")
	cmds.text(label=" UV方法：快速但需要相同的UV（也需要相同的集）而不重叠",  width=450, height=30, align="left", bgc=[0.79, 0.45, 0.45])
	cmds.text(label="              ",  width=450, height=5, align="left")
	cmds.text(label=" 字典方法：非常缓慢但准确，不依赖UV（推荐）",  width=450, height=30, align="left", bgc=[0.79, 0.45, 0.45])
	cmds.text(label="              ",  width=450, height=5, align="left")
	cmds.text(label="仅限非商业用途",  width=450, height=15, align="center", fn="boldLabelFont")
